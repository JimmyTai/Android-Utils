...
public class LinearProgressDrawable extends Drawable implements Animatable {
    ...
    private LinearProgressDrawable(float progressPercent, float secondaryProgressPercent, int maxLineWidth, float maxLineWidthPercent,
                                   int minLineWidth, float minLineWidthPercent, int strokeSize, int verticalAlign, int[] strokeColors, int
										   strokeSecondaryColor, boolean reverse, int travelDuration, int transformDuration, int
										   keepDuration, Interpolator transformInterpolator, int progressMode, int inAnimDuration, int
										   outAnimDuration) {
    ...
    private void updateQuery() {
        ...
        if (requestUpdate && mRunState != RUN_STATE_STOPPING) {
            ...
            mStartLine = mReverse ? mTransformInterpolator.getInterpolation(mAnimTime) * (width + minWidth) : ((1f -
					mTransformInterpolator.getInterpolation(mAnimTime)) * (width + minWidth) - minWidth);
        }
    ...
    public static class Builder {
        ...
        public Builder(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
            ...
            strokeSize(a.getDimensionPixelSize(R.styleable.LinearProgressDrawable_lpd_strokeSize, (int)JScreenUtils.dp2px(context, 4)));
