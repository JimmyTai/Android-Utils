package com.jimmytai.library.utils.material.drawable;
...
import com.jimmytai.library.utils.material.util.ColorUtil;
import com.jimmytai.library.utils.material.util.ThemeUtil;
...
public class CircularProgressDrawable extends Drawable implements Animatable {
    ...
    private CircularProgressDrawable(int padding, float initialAngle, float progressPercent, float secondaryProgressPercent, float
            maxSweepAngle, float minSweepAngle, int strokeSize, int[] strokeColors, int strokeSecondaryColor, boolean reverse, int
                                             rotateDuration, int transformDuration, int keepDuration, Interpolator transformInterpolator,
                                     int progressMode, int
                                             inAnimDuration, float inStepPercent, int[] inStepColors, int outAnimDuration) {
        mPadding = padding;
    ...
    private void drawDeterminate(Canvas canvas) {
        ...
        if (mRunState == RUN_STATE_STARTING) {
            size = (float) mStrokeSize * Math.min(mInAnimationDuration, (SystemClock.uptimeMillis() - mLastRunStateTime)) /
                    mInAnimationDuration;
            if (size > 0)
                radius = (Math.min(bounds.width(), bounds.height()) - mPadding * 2 - mStrokeSize * 2 + size) / 2f;
        } else if (mRunState == RUN_STATE_STOPPING) {
            size = (float) mStrokeSize * Math.max(0, (mOutAnimationDuration - SystemClock.uptimeMillis() + mLastRunStateTime)) /
    ...
    private void drawIndeterminate(Canvas canvas) {
        if (mRunState == RUN_STATE_STARTING) {
            ...
            float size = (float) mStrokeSize * Math.max(0, (mOutAnimationDuration - SystemClock.uptimeMillis() + mLastRunStateTime)) /
                    mOutAnimationDuration;
    ...
    private void start(boolean withAnimation) {
        ...
        scheduleSelf(mUpdater, SystemClock.uptimeMillis() + ProgressView.FRAME_DURATION);
        invalidateSelf();
    }
    ...
    public static class Builder {
        ...
        public Builder(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
            ...
            strokeSize(a.getDimensionPixelSize(R.styleable.CircularProgressDrawable_cpd_strokeSize, (int) JScreenUtils.dp2px(context, 4)));
            ...
            rotateDuration(a.getInteger(R.styleable.CircularProgressDrawable_cpd_rotateDuration, context.getResources().getInteger
                    (android.R.integer.config_longAnimTime)));
            transformDuration(a.getInteger(R.styleable.CircularProgressDrawable_cpd_transformDuration, context.getResources().getInteger
                    (android.R.integer.config_mediumAnimTime)));
            keepDuration(a.getInteger(R.styleable.CircularProgressDrawable_cpd_keepDuration, context.getResources().getInteger(android.R
            ...
            inAnimDuration(a.getInteger(R.styleable.CircularProgressDrawable_cpd_inAnimDuration, context.getResources().getInteger
                    (android.R.integer.config_mediumAnimTime)));
            ...
            outAnimDuration(a.getInteger(R.styleable.CircularProgressDrawable_cpd_outAnimDuration, context.getResources().getInteger
                    (android.R.integer.config_mediumAnimTime)));
            a.recycle();
        }
        ...
        public CircularProgressDrawable build() {
            ...
            return new CircularProgressDrawable(mPadding, mInitialAngle, mProgressPercent, mSecondaryProgressPercent, mMaxSweepAngle,
                    mMinSweepAngle, mStrokeSize, mStrokeColors, mStrokeSecondaryColor, mReverse, mRotateDuration, mTransformDuration,
                    mKeepDuration, mTransformInterpolator, mProgressMode, mInAnimationDuration, mInStepPercent, mInColors,
                    mOutAnimationDuration);
        }
